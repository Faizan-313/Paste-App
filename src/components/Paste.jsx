...
function Paste() {
    ...
    return (
        <div className='min-h-dvh'>
            <div className="bg-gradient-to-br from-gray-200  to-gray-300 shadow-2xl rounded-2xl p-8 mb-10 mx-auto max-w-3xl">
                <div className='mb-8'>
                ...
                <div className='space-y-6'>
                    {filteredPastes.length > 0 ? (
                        filteredPastes.map((paste) => (
                            <div key={paste?.pasteId} className='group relative bg-gradient-to-br from-gray-100  to-gray-300 hover:bg-white border border-gray-200 rounded-lg p-4 transition-all duration-200 hover:shadow-lg'>
                                <div className='flex flex-col sm:flex-row justify-between items-start mb-3 gap-2'>
                                    <h2 className='text-xl font-semibold text-gray-800 flex-1 min-w-0 break-words'>
                                        {paste.title}
                                    </h2>
                                    <div className='flex mb-3 mt-1 ml-3 gap-4'>
                                        <NavLink
                                ...
                                <div className='flex flex-col md:flex-row md:justify-between gap-3'>
                                    <pre className={`${paste.content === '---No Content---' ? 'text-red-500' : 'text-gray-700'} 
                                        whitespace-pre-wrap break-words font-mono text-sm 
                                        flex-1 overflow-hidden max-h-20 line-clamp-3`}>
                                        {paste.content.length > 57 ? `${paste.content.substring(0, 55)}...` : paste.content}
                                    </pre>
                                    <p className='flex justify-end gap-2 text-sm text-gray-500 md:self-end italic'>                                    <svg version="1.0" id="Layer_1"
